...
variable "cis_1_2_foundation_control_1_14_enabled" {
  type        = bool
  default     = false
...
variable "fsbp_standard_control_config_1_active_regions" {
  type        = list(string)
  default     = []
  description = "List of regions where the Config.1 FSBP control checks should be enabled. All other regions will have the control suppressed. If empty (default), no suppression rule is created."
}
...
variable "fsbp_standard_control_secretsmanager_1_enabled" {
  type        = bool
  default     = false
  description = "When false, disables Security Hub FSBP control for all secrets to have automatic rotation"
}
variable "guardduty_alert_minimum_severity" {
  type        = string
  description = "Minimum GuardDuty finding severity level to alert on. Alerts on this level and above. Valid values: low, medium, high, critical. Defaults medium."
  default     = "medium"
  validation {

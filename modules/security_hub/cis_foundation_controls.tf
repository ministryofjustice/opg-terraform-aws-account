...
locals {
  ...
  cis_controls = {
    ...
    cis_3_2_console_sign_in_without_mfa = {
      ...
      # pattern               = "{($.eventName = \"ConsoleLogin\") && ($.additionalEventData.MFAUsed != \"Yes\") && ($.additionalEventData.CredentialType != \"EXTERNAL_IDP\")}" T1 - did not work
      # pattern               = "{($.eventName=\"ConsoleLogin\") && ($.additionalEventData.MFAUsed !=\"Yes\")}" original control
